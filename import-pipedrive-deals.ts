...
async function main() {
  try {
    ...
    try {
      // Write the first deal to a debug file
      const debugFilePath = path.resolve(__dirname, 'pipedrive-debug.json');
      console.log(`Writing debug info to ${debugFilePath}`);
      
      // Extract just the relevant fields to keep the debug file smaller
      const debugData = {
        firstDeal: pipedriveDeals[0],
        firstDealOrg: pipedriveDeals[0].org_id
      };
    ...
    // For debugging purposes, let's query the organizations table to confirm what we have
    console.log('\nChecking organizations table in Supabase:');
    const { data: orgData, error: orgCheckError } = await supabase
      .from('organizations')
    ...
    if (orgCheckError) {
      console.error('Error querying organizations:', orgCheckError);
    } else {
      console.log(`Found ${orgData.length} organizations in Supabase`);
    ...
    for (const deal of pipedriveDeals) {
      try {
        ...
        // Extract the organization ID - it's the 'value' property in the org_id object
        let pipedriveOrgId = null;
        
        if (deal.org_id && typeof deal.org_id === 'object') {
          pipedriveOrgId = deal.org_id.value;
          console.log(`Organization: ${deal.org_id.name}, Pipedrive ID: ${pipedriveOrgId}`);
        } else {
          console.warn(`Cannot extract organization ID for deal: ${deal.title} - org_id: ${JSON.stringify(deal.org_id)}`);
          errorCount++;
          continue;
        }
        
        // Find matching organization in Supabase using the pipedrive_org_id field
        console.log(`Looking for organization with Pipedrive ID: ${pipedriveOrgId}`);
        ...
        // Map Pipedrive stage to our stage
