...
export default function DealCard({ deal }: DealCardProps) {
  ...
  // Very direct click handler - impossible to miss what's happening
  const handleCardClick = useCallback((e: React.MouseEvent) => {
    // Log a very noticeable message
    console.log('🚀🚀🚀 DEAL CARD CLICKED:', deal.id, deal.title);
    
    // Prevent bubbling
    e.preventDefault();
    e.stopPropagation();
    
    // Navigate directly - should work regardless of router issues
    ...
    >
      <CardContent className="p-3">
        ...
        <Button 
          ...
          onClick={handleCardClick}
        >
          <Eye className="h-3 w-3 mr-1" /> View Deal Details
        </Button>
      </CardContent>
